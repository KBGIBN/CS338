...
frame_size = (640,480)
IMG_PATH = 'D:/UIT/CS338/face_id/data/test_images'
DATA_PATH = 'D:/UIT/CS338/face_id/data'
...
def inference(model, face, local_embeds, threshold = 3):
    ...
    vector_a = detect_embeds.detach().numpy()
    vector_b = torch.transpose(local_embeds,0,1).detach().numpy()
    #print(vector_a.shape)
    #print(vector_b.shape) 
    #norm_diff = detect_embeds.unsqueeze(-1) - torch.transpose(local_embeds, 0, 1).unsqueeze(0)
    norm_diff = dot(vector_a, vector_b)/(norm(vector_a)*norm(vector_b))
    #print(norm_diff)
